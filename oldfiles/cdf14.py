...
def plot_and_save(times, intensity, unit,
    ...
    # stops ticks and tick labels on y axis on the inset
...
def main():
    ...
    # Write main chromatogram to CSV
    out_csv = os.path.splitext(args.input)[0] + "_chromatogram.csv"
    np.savetxt(
        out_csv,
        np.column_stack((times, intensity)),
        delimiter=",",
        header=f"time_{unit},intensity",
        comments=""
)
