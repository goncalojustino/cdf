...
def plot_chromatogram(times, intensity, unit, subtract_baseline=False, output=None):
    """
    Plot chromatogram, marking the highest-intensity peak with an offset cross and label.
    """
    ...
    # Plot chromatogram signal (no legend)
    ...
    # Compute small horizontal offset (0.5% of time range)
    time_span = times.max() - times.min()
    dx = time_span * 0.005
    marker_x = tr_max + dx
    marker_y = h_max

    # Plot cross marker offset to the right
    ax.scatter([marker_x], [marker_y], marker='x')
    # Annotate below and to the right of marker
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Plot CDF chromatogram with highest peak marker.'
    )
    parser.add_argument(
