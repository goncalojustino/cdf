...
USAGE
  ...
  python cdf13_post.py kh18.cdf \
      --smooth 8 --snr 2 --noise-mode pre --noise 5000 --noise-window 0.5 \
      --min-width-sec 1 --reject-height 0 --min-prominence 0 \
      ...
      --fill-alpha 0.5 --fill-color "#9ecae1" --baseline-lw 1.2 --baseline-color black
...
def simpson_integrate(t: np.ndarray, y: np.ndarray) -> float:
    ...
    for i in range(0, n - 2, 2):
        ...
        last_used = i + 2
    if last_used < n - 1:
        area += np.trapezoid(y[last_used:], t[last_used:])
    return float(area)
...
def main():
    ...
    # Detection
    p.add_argument("--smooth", type=int, default=8, help="moving average window (points) for detection; default 32")
    p.add_argument("--snr", type=float, default=2.0, help="minimum S/N for peak acceptance; default 5")
    p.add_argument("--noise-mode", choices=["fixed", "start", "pre"], default="pre",
    ...
    p.add_argument("--noise-window", type=float, default=0.3, help="minutes used for start/pre noise estimation; default 0.5")
    p.add_argument("--min-width-sec", type=float, default=1.0, help="minimum peak width valley-to-valley (sec); default 4")
    ...
    p.add_argument("--reject-height", type=float, default=0.0,
    ...
    p.add_argument("--fill-alpha", type=float, default=0.5, help="peak fill opacity (0–1)")
    p.add_argument("--fill-color", default="#9ecae1", help="peak fill color (e.g., '#9ecae1')")
    p.add_argument("--baseline-lw", type=float, default=1.2, help="baseline line width")
    p.add_argument("--baseline-color", default="black", help="baseline color")
    args = p.parse_args()
    ...
    def estimate_start_noise():
        ...
        mask = (t <= t_end)
        return float(np.std(y_det[mask])) if mask.any() else float(args.noise)
    ...
    for pidx in peaks_sep:
        ...
        if args.noise_mode == "pre":
            ...
            noise = float(np.std(y_det[mask_pre])) if mask_pre.sum() >= 5 else float(args.noise)
        else:
    ...
    def integrate(seg_t: np.ndarray, seg_y: np.ndarray) -> float:
        ...
        return float(np.trapezoid(seg_y, seg_t))  # numpy >=1.20
    ...
    for k, (iL0, p, iR0) in enumerate(merged, start=1):
        ...
        measure_value = (
            area if args.measure == "area"
            else apex_height if args.measure == "height"
            else float(np.sqrt(max(apex_height, 0.0)))
        )
    ...
    summary = {
        ...
        "params": {
            ...
            "fill_alpha": float(args.fill_alpha),
            "fill_color": args.fill_color,
            "baseline_lw": float(args.baseline_lw),
    ...
    # draw baseline segments and fills for each integrated peak
    for row in peaks_rows:
        tL, tR, rt = row["t_start_min"], row["t_end_min"], row["rt_min"]
        ...
        # choose plotting baseline method to match integration
        if args.baseline_method == "valley":
            base_vals = segment_baseline_valley(t, y_raw, iL, iR)
            bL_i, bR_i = iL, iR
        else:
            # need apex index for tangent method
            p = int(np.argmin(np.abs(t - rt)))
            base_vals, bL_i, bR_i = segment_baseline_tangent(t, y_raw, iL, p, iR, float(args.tangent_height_pct))
            seg_t = t[bL_i:bR_i+1]
            seg_y = y_raw[bL_i:bR_i+1]

        # baseline line behind the trace
        ax.plot([t[bL_i], t[bR_i]], [y_raw[bL_i], y_raw[bR_i]],
                color=args.baseline_color, lw=float(args.baseline_lw), alpha=0.9, zorder=1)

        # filled area
        ax.fill_between(seg_t, seg_y, base_vals,
                        where=(seg_y > base_vals),
                        color=args.fill_color, alpha=float(args.fill_alpha),
                        linewidth=0, interpolate=True, clip_on=True, zorder=1.2)
