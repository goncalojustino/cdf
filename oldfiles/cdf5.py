...
def plot_chromatogram(times, intensity, unit, subtract_baseline=False, output=None):
    """
    Plot chromatogram, marking the highest-intensity peak.
    """
    ...
    # Marker on highest peak
    ax.scatter([tr_max], [h_max], marker='o')
    # Annotate with retention time
    ax.annotate(f'tr = {tr_max:.2f} min',
                xy=(tr_max, h_max), xytext=(5, 5), textcoords='offset points',
                ha='left', va='bottom')
